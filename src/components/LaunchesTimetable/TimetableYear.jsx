 ...
 * @param number year
 */
const launchesByYear = (launches, year) =>
...
const launchClassName = (launch) => {
  ...
  if (launch.launch_success === false)
    classes.push('timetable__event_unsuccessfully');
...
const TimetableYear = ({ launches, year, displayLaunch }) => {
  ...
  return (
    <div className={`timetable-year${year % 2 ? ' timetable-year_odd' : ''}`}>
      <span
        className={`timetable-year__label${
          year % 2 ? ' timetable-year__label_odd' : ''
        }`}
      >
        {year}
      </span>
      {monthsNamesShort.map((monthName, monthNumber) => (
        <div
          ...
          className={`timetable__month${
            now.getMonth() === monthNumber && now.getFullYear() === year
              ? ' timetable__month_current'
              : ''
          }`}
        >
          <span
            className={`timetable__month-name${
              now.getMonth() === monthNumber && now.getFullYear() === year
                ? ' timetable__month-name_current'
                : ''
            }`}
