...
describe('FilterByRocket', () => {
  it('renders all uniq rockets from launches', () => {
    ...
    expect(wrapper.find('button').at(0).text()).toEqual('First Rocket');
    expect(wrapper.find('button').at(1).text()).toEqual('Second Rocket');
    expect(wrapper.find('button').at(2).text()).toEqual('Third Rocket');
  });
